...
The instructions bellow assume you have the `mvn` command available in you PATH. If you do not wish to install Maven, you can use the provided wrapper by replacing the `mvn` command with either `mvnw.cmd` (on Windows) or `./mvnw` (other operating systems).
